...
<head>
    ...
    <title>Quantum Audiobook Library - Quantum Minds United</title>
    <style>
        :root {
            --primary-bg: #0a0f1c;
            --secondary-bg: #1a1f35;
    ...
    <div class="container">
        <div class="header">
            <div class="header-content">
                ...
                <h1>📚 Digital Audiobook Collection</h1>
                <p class="subtitle">Quantum Minds Collection</p>
                <p class="subtitle">Works Created by Meistro</p>
            </div>
        </div>
        ...
        <?php if (empty($audioFiles)): ?>
            <div class="no-files">
                ...
                <p>Make sure you have .mp3 files in this directory to start your reading journey.</p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            audios.forEach((audio, index) => {
                ...
                // Restore reading position - commented out to avoid localStorage issues
                // audio.addEventListener('loadedmetadata', function() {
                //     const savedTime = sessionStorage.getItem(`audiobook_${index}_time`);
                //     if (savedTime) {
                //         this.currentTime = parseFloat(savedTime);
                //     }
                // });
                
                // Store reading position - using sessionStorage instead
                // audio.addEventListener('timeupdate', function() {
                //     if (this.currentTime > 0) {
                //         sessionStorage.setItem(`audiobook_${index}_time`, this.currentTime);
                //     }
                // });
            });
        });
        ...
        function updateProgress(audio, index) {
            ...
            const progressBar = document.querySelector(`[data-index="${index}"] .progress-indicator`);
            if (progressBar) {
        ...
        function playAll() {
            if (library.length > 0) {
                const firstBook = document.getElementById(`audiobook_${library[0]}`);
                firstBook.play();
            }
        }
        ...
        function sortByDate() {
            ...
            // Update library indices after reordering
            updateLibraryIndices();
        }
        
        function updateLibraryIndices() {
            const audiobookItems = document.querySelectorAll('.audiobook');
            library = [];
            
            audiobookItems.forEach((item, newIndex) => {
                const originalIndex = parseInt(item.dataset.index);
                library.push(originalIndex);
                
                // Update the visual index for proper tracking
                item.dataset.visualIndex = newIndex;
            });
        }
        
        function previousBook() {
            if (currentBook > 0) {
                const prevIndex = library.indexOf(currentBook) - 1;
                if (prevIndex >= 0) {
                    const prevBook = library[prevIndex];
                    document.getElementById(`audiobook_${prevBook}`).play();
                }
            }
        }
        ...
        function previousBook() {
            if (currentBook >= 0) {
                const currentLibraryIndex = library.indexOf(currentBook);
                if (currentLibraryIndex > 0) {
                    const prevBook = library[currentLibraryIndex - 1];
                    document.getElementById(`audiobook_${prevBook}`).play();
                }
            }
        }
        ...
        function nextBook() {
            if (currentBook >= 0) {
                const currentLibraryIndex = library.indexOf(currentBook);
                if (currentLibraryIndex < library.length - 1) {
                    const nextBook = library[currentLibraryIndex + 1];
                    document.getElementById(`audiobook_${nextBook}`).play();
                } else {
                    // End of library
                    pauseAll();
                }
